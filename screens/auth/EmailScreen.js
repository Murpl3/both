...
const EmailScreen = () => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        await associateWalletWithUser(formattedPhone);
      }
      ...
      navigation.navigate('VerifyOTP', { phoneNumber: formattedPhone, mode, backendUrl });
    } catch (err) {
  ...
  return (
    <Screen>
      <AppHeader title="Recover access" onBack={() => navigation.goBack()} />
      <KeyboardAvoidingView
        style={styles.flex}
        ...
        >
          <Card radius="large" style={styles.card}>
            ...
            <PrimaryButton
              ...
              icon={<Feather name="arrow-right" size={20} color={RT.white} />}
            />
          </Card>
        </ScrollView>
      </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  title: {
    fontSize: Math.min(SCREEN_WIDTH * 0.07, 26),
